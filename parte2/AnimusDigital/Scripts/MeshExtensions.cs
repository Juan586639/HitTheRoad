...
public static class MeshExtensions  {
    ...
    public static Mesh SeparateVerts(this Mesh mesh, params int[] indices)
    {
        ...
        for (int i = 0; i < mesh.subMeshCount; i++) Tris.Add(mesh.GetTriangles(i));
        ...
        var MeshTris = new List<Triangle>();
        for (int i = 0; i < Verts.Length; i++) Vertices.Add(new Vertex() { GroupID=0, Coord = Verts[i], boneWeight = BoneWeights[i], UVCoord = UVs[i], Normal = Normals[i] });
        for (int submesh=0; submesh<Tris.Count; submesh++)
            for (int i = 0; i < Tris[submesh].Length; i+=3)
                MeshTris.Add(new Triangle() { first = Vertices[Tris[submesh][i]], second = Vertices[Tris[submesh][i + 1]], third = Vertices[Tris[submesh][i + 2]] , SubMeshIndex=submesh});

        //3. Separate triangles by GroupID
        for (int i = 0; i < indices.Length; i++) Vertices[indices[i]].GroupID = 1;
        var Separation = MeshTris.FindAll(t => t.first.GroupID == 1 && t.second.GroupID == 1 && t.third.GroupID == 1);
        MeshTris = MeshTris.FindAll(t => t.first.GroupID == 0 && t.second.GroupID == 0 && t.third.GroupID == 0);

        //4. Convert list of vertices back to arrays
        var MeshVerts = new Vector3[Vertices.Count - indices.Length];
        var MeshUVs = new Vector2[Vertices.Count - indices.Length];
        var MeshBoneWeights = new BoneWeight[Vertices.Count - indices.Length];
        var MeshNormals = new Vector3[Vertices.Count - indices.Length];
        int MeshIdx = 0;
        ...
        {
            mesh.SetTriangles(SubmeshedTris[i].ToArray(),i);
        }
        ...
        for (int i = 0; i < submeshes; i++) SubmeshedTris[i] = new List<int>();
        for (int i = 0; i < Separation.Count; i++)
        {
        ...
        for (int i = 0; i < submeshes; i++)
        {
            SeparatedMesh.SetTriangles(SubmeshedTris[i].ToArray(), i);
        }
